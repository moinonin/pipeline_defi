...
def process_strategy(strat_ver: str, data_size: int):
    ...
    data = pd.read_csv(f'/home/defi/Desktop/portfolio/projects/python/pipeline_defi/clean_data/imitate_small_{strat_ver}.csv') #  ../jupyter/spreadsheets/tuned-indicators.csv
    #print(data.head())
    ...
    return train_data.to_csv(f'../jupyter/spreadsheets/rlhf_small_{strat_ver}.csv')

def main_small_proc(strat_ver: str, data_size: int = None):
    return process_strategy(strat_ver, data_size)

if __name__ == '__main__':
    fire.Fire(main_small_proc)
